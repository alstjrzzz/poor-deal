...
<head>
    ...
    <style>
        ...
        .mail-card { border: none; border-radius: 15px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); background: white; overflow: hidden; }
        .table thead th { background-color: #f1f3f5; border-bottom: 2px solid #dee2e6; font-weight: 600; color: #495057; text-align: center; vertical-align: middle; }
        .table tbody td { vertical-align: middle; }
        .mail-link { display: block; color: #212529; text-decoration: none; font-weight: 500; }
        .mail-link:hover { color: #0d6efd; }
        .pagination .page-link { border: none; color: #495057; margin: 0 3px; border-radius: 50%; width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; }
        .pagination .page-item.active .page-link { background-color: #0d6efd; color: white; font-weight: bold; }
    </style>
</head>
<body>
                            ...
                            <c:forEach var="mail" items="${mailList}">
                                <tr>
                                    <td class="text-center">
                                        <c:choose>
                                            <c:when test="${mail.processType == 'TRADE'}"><span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill px-2">🛒 거래</span></c:when>
                                            <c:when test="${mail.processType == 'RECRUIT'}"><span class="badge bg-warning bg-opacity-10 text-dark border border-warning border-opacity-25 rounded-pill px-2">🤝 구인</span></c:when>
                                            <c:otherwise><span class="badge bg-light text-secondary border px-2">📢 일반</span></c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <a href="/mail/${mail.id}" class="mail-link text-truncate" style="max-width: 300px;">
                                            <c:if test="${mail.senderId == loginId}"><span class="badge bg-secondary me-1" style="font-size: 0.7rem;">MY</span></c:if>
                                            ${mail.title}
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        <c:choose>
                                            <c:when test="${mail.senderId == loginId}">
                                                <span class="text-muted small"><i class="bi bi-arrow-right-short text-primary"></i> ${mail.receiver}</span>
                                            </c:when>
                                            <c:otherwise>
